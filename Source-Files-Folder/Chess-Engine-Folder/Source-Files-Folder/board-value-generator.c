...
int check_mate_value(CBoard board, Team team)
{
  if(!piece_team_exists(team)) return MIN_VAL;
  ...
  // If the teamKing (own king) is in check mate
	if(check_mate_situation(board, team))        value += MIN_VAL;

	// If the enemyKing (opponent) is in check mate
	else if(check_mate_situation(board, enemy))  value += MAX_VAL;

	return value;
}

int game_draw_value(CBoard board, Team team)
{
  if(!piece_team_exists(team)) return MIN_VAL;
	...
	if(check_draw_situation(board, team))        value += MAX_VAL;

	// If the opponent cant move (you did draw)
	else if(check_draw_situation(board, enemy))  value += MIN_VAL;

	return value;
}

int board_state_value(CBoard board, Team team)
{
	int value = 0;

	value += team_pieces_value(board.board, team);

	value += check_mate_value(board, team);

	value += game_draw_value(board, team);

	return value;
}

int team_state_value(CBoard board, Team team)
{
  if(!piece_team_exists(team)) return MIN_VAL;

	int enemy = (team == WHITE) ? BLACK : WHITE;

	int enemyValue = board_state_value(board, enemy);
	int teamValue = board_state_value(board, team);
