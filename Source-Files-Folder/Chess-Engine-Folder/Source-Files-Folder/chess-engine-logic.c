...
bool board_points_enemies(Board board, Point first, Point second)
{
  Team firstTeam = board_point_team(board, first);
  Team secondTeam = board_point_team(board, second);

  return board_pieces_enemies(firstTeam, secondTeam);
}
...
bool board_pieces_enemies(Team first, Team second)
{
  bool teamExists = piece_team_exists(first);
